...
public abstract class SimulationManager extends Parameters
{
    ...
    public SimulationManager(int numCells, int guiCellWidth, double maxTime)
    {
        ...
        Cell.setRateMean(REGROWTH_RATE_MEAN);
        Cell.setRateSD(REGROWTH_RATE_SD);
        Cell.setResourceMean(MAX_RESOURCE_MEAN);
        Cell.setResourceSD(MAX_RESOURCE_SD);

        Bacterium.setConsumptionMean(CONSUMPTION_RATE_MEAN);
        Bacterium.setRateSD(CONSUMPTION_RATE_SD);
        Bacterium.setInitMean(INIT_RESOURCE_MEAN);
        Bacterium.setResourceSD(INIT_RESOURCE_SD);
        Bacterium.setInterMove(BACT_INTER_MOVE);
        Bacterium.setInterDivide(BACT_INTER_DIVIDE);

        Macrophage.setInterMove(MACRO_INTER_MOVE);
        Macrophage.setInterDivide(MACRO_INTER_DIVIDE);
        Macrophage.setMinBact(MIN_BACT_TO_DIVIDE);
